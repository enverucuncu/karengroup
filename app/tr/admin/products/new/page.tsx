...
export default function NewProduct(){
  ...
  async function onSubmit(e:React.FormEvent<HTMLFormElement>){
    ...
    const r = await fetch('/api/admin/products', { method:'POST', body: fd }); const j = await r.json(); alert(j.ok?'Kaydedildi':'Hata: '+j.error)
  }
  return (
    <div className="container section">
      <h1 className="h2 mb-6">Yeni Ürün (TR giriş)</h1>
      <form onSubmit={onSubmit} className="grid gap-4 max-w-2xl">
        ...
        <textarea name="descTr" placeholder="Açıklama (TR)" className="px-3 py-2 rounded bg-black/30 border border-white/10" required/>
        <textarea name="specsTr" placeholder="Teknik Özellikler (TR - HTML)" className="px-3 py-2 rounded bg-black/30 border border-white/10" required/>
